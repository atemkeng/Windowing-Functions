...
class MSResampler (object):
    ...
    def boxcar (self,dtime,dfreq):
	 ...
	 # import Pyxis;
	 # info("extract full array for  baseline (%i,%i), apply subset in time"%(p,q))
	  ...
	  data = data[self.time0:self.time0+self.ntime,self.freq0:self.freq0+self.nfreq,:];
...
##      wl = self.wavel.reshape((nfreq1,dfreq));
      # get frequency and wavelength (per channel) from the CHAN_FREQ
      freqs = self.channels[0,self.freq0:self.freq0+self.nfreq];
      wavel = 3e8/freqs;
      wl = wavel.reshape((nfreq1,dfreq));
	  ...
	  data = data[self.time0:self.time0+self.ntime,self.freq0:self.freq0+self.nfreq,:];
          ...
          uvd1 = np.sqrt(((uv.reshape((ntime1,dtime,2)) - uv0[:,np.newaxis,:])**2).sum(2))#2
	 ...
	 # import scipy.signal
          ...
          wf = window_function(uvd1[:,:,:,np.newaxis],uvd2[:,np.newaxis,:,:])#*selepian;
          if dump:
            import cPickle
